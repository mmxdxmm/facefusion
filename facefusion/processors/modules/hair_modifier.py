...
from facefusion.common_helper import create_int_metavar, create_int_range
...
from facefusion.typing import ApplyStateItem, Args, Face, FaceSet, InferencePool, Mask, ModelOptions, ModelSet, ProcessMode, QueuePayload, UpdateProgress, VisionFrame
...
MODEL_SET : ModelSet =\
{
	'styleganex_hair':
	{
		'hashes':
		{
			'hair_modifier':
			{
				'url': 'https://huggingface.co/henryruhs/models/resolve/main/6e9f618.hash',
				'path': resolve_relative_path('../.assets/models/6e9f618.hash')
			}
		},
		'sources':
		{
			'hair_modifier':
			{
				'url': 'https://huggingface.co/henryruhs/models/resolve/main/6e9f618.onnx',
				'path': resolve_relative_path('../.assets/models/6e9f618.onnx')
			}
		},
		'template': 'ffhq_512',
		'size': (512, 512)
...
HairModifierInputs = TypedDict('HairModifierInputs',
{
	'reference_faces' : FaceSet,
	'target_vision_frame' : VisionFrame
})

processors_choices.hair_modifier_models = [ 'styleganex_hair' ]
processors_choices.hair_modifier_direction_range = create_int_range(-100, 100, 1)
...
def get_model_options() -> ModelOptions:
	hair_modifier_model = state_manager.get_item('hair_modifier_model')
	return MODEL_SET.get(hair_modifier_model)


def register_args(program : ArgumentParser) -> None:
	group_processors = find_argument_group(program, 'processors')
	if group_processors:
		group_processors.add_argument('--hair-modifier-model', help = wording.get('help.hair_modifier_model'), default = config.get_str_value('processors.hair_modifier_model', 'styleganex_hair'), choices = processors_choices.hair_modifier_models)
		group_processors.add_argument('--hair-modifier-direction', help = wording.get('help.hair_modifier_direction'), type = int, default = config.get_int_value('processors.hair_modifier_direction', '0'), choices = processors_choices.hair_modifier_direction_range, metavar = create_int_metavar(processors_choices.hair_modifier_direction_range))
		facefusion.jobs.job_store.register_step_keys([ 'hair_modifier_model', 'hair_modifier_direction' ])


def apply_args(args : Args, apply_state_item : ApplyStateItem) -> None:
	apply_state_item('hair_modifier_model', args.get('hair_modifier_model'))
	apply_state_item('hair_modifier_direction', args.get('hair_modifier_direction'))
...
def modify_hair(target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
...
def forward(crop_vision_frame : VisionFrame, extend_vision_frame : VisionFrame) -> VisionFrame:
	hair_modifier = get_inference_pool().get('hair_modifier')
	hair_modifier_inputs = {}

	for hair_modifier_input in hair_modifier.get_inputs():
		if hair_modifier_input.name == 'target':
			hair_modifier_inputs[hair_modifier_input.name] = crop_vision_frame
		if hair_modifier_input.name == 'target_with_background':
			hair_modifier_inputs[hair_modifier_input.name] = extend_vision_frame
		if hair_modifier_input.name == 'direction':
			hair_modifier_inputs[hair_modifier_input.name] = prepare_direction(state_manager.get_item('hair_modifier_direction'))

	with thread_semaphore():
		crop_vision_frame = hair_modifier.run(None, hair_modifier_inputs)[0][0]
...
def prepare_direction(direction : int) -> NDArray[Any]:
	direction = numpy.interp(float(direction), [ -100, 100 ], [ 2, -2 ]) #type:ignore[assignment]
...
def get_reference_frame(source_face : Face, target_face : Face, temp_vision_frame : VisionFrame) -> VisionFrame:
	return modify_hair(target_face, temp_vision_frame)


def process_frame(inputs : HairModifierInputs) -> VisionFrame:
	...
	if state_manager.get_item('face_selector_mode') == 'many':
		if many_faces:
			for target_face in many_faces:
				target_vision_frame = modify_hair(target_face, target_vision_frame)
	...
	if state_manager.get_item('face_selector_mode') == 'reference':
		...
		if similar_faces:
			for similar_face in similar_faces:
				target_vision_frame = modify_hair(similar_face, target_vision_frame)
